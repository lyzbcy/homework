...
class MedicalGraph:
    ...
    def create_relationship(self, start_node, end_node, edges, rel_type, rel_name):
        ...
        for edge in set(set_edges):
            ...
            p = edge[0]
            q = edge[1]
            batch.append({'p': p, 'q': q})
            
            if len(batch) >= batch_size:
                query = "UNWIND $batch as row MATCH (p:%s), (q:%s) WHERE p.name=row.p AND q.name=row.q MERGE (p)-[rel:%s{name:'%s'}]->(q)" % (
                    start_node, end_node, rel_type, rel_name)
                try:
